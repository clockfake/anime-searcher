...
function sortDates(a: Note, b: Note): number {
...
export default class NotesList extends Component<Props, State> {
  ...
  handleDelete = (id: string) => {
    ...
    if (notesArr && notesArr.length) {
      this.setState({ notesArr: notesArr.filter(note => note.id !== id) });
    }
  }
  ...
  render() {
    const { notesArr } = this.state;
    if (!notesArr || !notesArr.length) {
      return <div>No notes yet. Try to add some notes for anime you watched.</div>;
    }
    return (
      <div className="card__container">
        {notesArr.sort(sortDates).map(note => (
          <div className="card m-2" key={note.id}>
            ...
            <div className="card-footer">
              ...
              <button type="button" className="card-link btn btn-link" onClick={() => this.handleDelete(note.id)}>Delete note</button>
            </div>
          </div>
        ))}
      </div>
